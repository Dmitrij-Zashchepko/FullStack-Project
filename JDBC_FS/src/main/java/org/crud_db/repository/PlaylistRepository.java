...
public class PlaylistRepository {
    ...
    public List<Playlist> getAllPlaylists() throws SQLException {
        ...
        String sql = "SELECT playlist_id, user_id, title, description, created_at, is_public FROM playlists";
    ...
    public Playlist getPlaylistById(int playlistId) throws SQLException {
        String sql = "SELECT playlist_id, user_id, title, description, created_at, is_public " +
                     "FROM playlists WHERE playlist_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, playlistId);
            try (ResultSet rs = stmt.executeQuery()) {
                return rs.next() ? mapResultSetToPlaylist(rs) : null;
            }
        }
    }
